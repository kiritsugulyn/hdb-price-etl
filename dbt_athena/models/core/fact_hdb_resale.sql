...
select 
    ...
    hdb_resale_price.year_num as year,
    hdb_resale_price.month_num as month,

    -- location info
    {{ get_region('hdb_resale_price.town') }} as region,
    upper(hdb_resale_price.town) as town,
    upper(hdb_resale_price.street_name) as street,
    upper(hdb_resale_price.block) as block,
    concat('BLK ', upper(hdb_resale_price.block), ' ', upper(hdb_resale_price.street_name)) as address,

    -- flat info
    upper(regexp_replace(hdb_resale_price.flat_type, '-',' ')) as flat_type,
    upper(regexp_replace(hdb_resale_price.flat_model, '-',' ')) as flat_model,
    ...
    {{ get_remaining_lease("hdb_resale_price.remaining_lease", "hdb_resale_price.lease_commence_date", "hdb_resale_price.year_num") }} as lease_remain_year,
...
left join hdb_property_info 
    on upper(hdb_resale_price.street_name) = upper(hdb_property_info.street)
    and upper(hdb_resale_price.block) = upper(hdb_property_info.blk_no)
